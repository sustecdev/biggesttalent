<main class="flex-grow flex justify-center items-start p-3 sm:p-6 lg:p-8 py-6 sm:py-12 relative overflow-x-hidden overflow-y-auto min-h-0" style="padding-top: max(1.5rem, env(safe-area-inset-top)); padding-bottom: max(1.5rem, env(safe-area-inset-bottom)); padding-left: max(0.75rem, env(safe-area-inset-left)); padding-right: max(0.75rem, env(safe-area-inset-right));">
    ...
    <div class="w-full max-w-md relative z-10 auth-card-wrapper">
        <div class="bg-[rgba(0,0,0,0.3)] backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-5 sm:p-8 lg:p-10 relative w-full">
            <div class="mb-6 sm:mb-8 relative z-10">
                <h2 class="text-2xl sm:text-3xl font-bold text-white mb-2 tracking-tight">Create Account</h2>
            ...
            <form id="signup-form" class="space-y-4 relative z-10">
                ...
                <div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-2">
                        ...
                        <button type="button" id="send-otp-btn"
                            class="min-h-[48px] px-4 py-3.5 bg-white/10 hover:bg-white/20 active:bg-white/25 border border-pink-300/30 rounded-xl text-pink-100 text-sm font-medium whitespace-nowrap transition-all touch-manipulation">
                            Send Code
                        </button>
                    </div>
                    <div id="otp-section" class="mt-3" style="display: none;">
                        ...
                        <div class="flex flex-col sm:flex-row gap-2">
                            <input type="text" id="otp" name="otp" maxlength="6" inputmode="numeric" pattern="[0-9]{6}"
                                class="flex-1 min-h-[48px] sm:min-h-0 bg-white/10 border border-pink-300/30 rounded-xl py-3.5 px-4 text-white placeholder-pink-200 focus:outline-none focus:ring-2 focus:ring-white focus:border-transparent text-center text-xl font-mono tracking-widest"
                                placeholder="000000">
                            <button type="button" id="verify-otp-btn"
                                class="min-h-[48px] px-4 py-3.5 bg-[#cd217d] hover:bg-[#a51a64] active:bg-[#8b1550] border border-white/20 rounded-xl text-white text-sm font-medium whitespace-nowrap transition-all touch-manipulation">
                                Verify
                            </button>
                        </div>
                ...
                <button type="submit" id="signup-btn" disabled
                    class="w-full min-h-[52px] bg-[#cd217d] text-white font-bold py-4 px-4 rounded-xl shadow-lg hover:bg-[#a51a64] active:bg-[#8b1550] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-2 group mt-4 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 touch-manipulation text-base">
                    Next: Set Master PIN
...
<!-- Coming Soon Modal -->
<div id="coming-soon-modal" class="fixed inset-0 bg-black/70 flex justify-center items-center z-[9999] p-4" style="display: none; padding: max(1rem, env(safe-area-inset));">
    <div class="bg-white p-6 sm:p-10 rounded-2xl text-center max-w-[90vw] w-full sm:max-w-md sm:w-[450px] shadow-2xl">
        <h2 class="text-xl sm:text-2xl font-bold text-gray-900 mb-6 sm:mb-8 uppercase tracking-wide">2026 biggest talent africa <br> season is coming soon</h2>
        <button id="close-modal-btn" class="min-h-[48px] bg-[#cd217d] text-white font-bold py-3 px-8 rounded-xl hover:bg-[#a51a64] active:bg-[#8b1550] touch-manipulation">Okay</button>
    </div>
</div>
